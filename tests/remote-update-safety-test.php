...
namespace {
	...
	define( 'AG_SYNC_BRIDGE_VERSION', '0.1.36' );
...
namespace {
	...
	$sha = str_repeat( 'a', 64 );
	$result = $service->update_from_github_release( '0.1.37', $sha, '0.1.36', 'WRONG' );
	...
	$result = $service->update_from_github_release( '0.1.37', 'bad', '0.1.36', 'UPDATE AG SYNC' );
	...
	$result = $service->update_from_github_release( '0.1.37', $sha, '0.1.35', 'UPDATE AG SYNC' );
	...
	$result = $service->update_from_github_release( '0.1.35', $sha, '0.1.36', 'UPDATE AG SYNC' );
	...
	$result = $service->update_from_github_release( '0.1.37', $sha, '0.1.36', 'UPDATE AG SYNC' );
	...
	expect_update(
		$service->official_url( 'https://github.com/andrea52k/wordpress-plugin-ag-sync-bridge/releases/download/v0.1.37/ag-sync-bridge.zip', '0.1.37' ),
		'official asset URL accepted'
	);
	expect_update(
		! $service->official_url( 'https://example.com/ag-sync-bridge.zip', '0.1.37' ),
	...
	$zip->close();
	expect_update( true === $service->validate_test_package( $valid_path, '0.1.36' ), 'valid canonical package accepted' );
	expect_update( is_wp_error( $service->validate_test_package( $valid_path, '0.1.37' ) ), 'version mismatch rejected' );
	...
	$zip->close();
	$unsafe = $service->validate_test_package( $unsafe_path, '0.1.36' );
